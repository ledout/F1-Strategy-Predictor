...
def get_latest_completed_race():
    """
    Finds the most recent F1 event or upcoming one.
    ...
    for year in sorted(YEARS, reverse=True):
        try:
            ...
            if 'EventDate' in schedule.columns:
                 ...
                 # Filter conventional
                 valid_events = schedule.loc[schedule['EventFormat'] == 'conventional']
                 
                 if not valid_events.empty:
                    # 1. Look for upcoming race (next 4 days)
                    ...
                    # 2. Look for last completed race
